...
│  政府電子採購網（工程/營運招標、決標） │              │
└─────────────────────────────────────┘              │
        相關性過濾 → 判區(37區) → 分類 → 座標 → 去重合併 → 標題清理
        → 回填舊資料判區 → 事件聚合（同一件事的多家報導歸成一群）
                                                     │
                                            data/news.json（版本控管）
                                                     │
...
`data/news.json` 的每一則與舊版 Apps Script 試算表欄位相同，再多三個欄位：

```json
{ "id": "…", "title": "…", "source": "…", "url": "…", "category": "新開店",
  "date": "ISO 8601", "snippet": "…", "district": "中西區", "lat": 22.99, "lng": 120.19,
  "story": "代表那則的 id（同一件事的多家報導共用）",
  "firstSeen": "第一次被蒐集到的時間（前端用來標「新進」）" }
```

檔案頂層另有 `lastFetched`（上次執行時間）與 `addedLastRun`（上次新增的 id 清單）。
